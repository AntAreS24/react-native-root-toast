...
Keyboard.addListener('keyboardDidChangeFrame', function ({ endCoordinates }) {
  // Dynamic update of the height
  KEYBOARD_HEIGHT = Dimensions.get('window').height - endCoordinates.screenY;
});
...
class ToastContainer extends Component {
  ...
  _onLayout = (evt) => {
    this.setState({ deviceHeight: evt.nativeEvent.layout.height, deviceWidth: evt.nativeEvent.layout.width })
  }
      ...
      >
        <Animated.View
          style={[
            ...
            { marginHorizontal: this.state.deviceWidth * ((1 - TOAST_MAX_WIDTH) / 2) },
